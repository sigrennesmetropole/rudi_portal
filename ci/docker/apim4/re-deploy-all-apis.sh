...
apis=$(curl -u "admin:admin" --insecure "https://${APIM_HOST}:${APIM_PORT}/api/am/publisher/v4/apis")
...
change-lifecycle() {
  curl -u "admin:${APIM_PASSWORD}" --insecure --request POST "https://${APIM_HOST}:${APIM_PORT}/api/am/publisher/v4/apis/change-lifecycle?action=$1&apiId=$2" -o /dev/null -w "HTTP Status : %{http_code}\n"
}
