...
public abstract class ResourcesHelper {
	...
	public DocumentContent loadResources(String resourceUuid) throws IOException {
		...
		if (f.exists() && f.isFile()) {
			...
			// convertir l'extension en content type
			result = new DocumentContent(f.getName(), extension, f);
		} else {
			result = DocumentContent.fromResourcePath(getBasePackage() + resourceName);
		}
