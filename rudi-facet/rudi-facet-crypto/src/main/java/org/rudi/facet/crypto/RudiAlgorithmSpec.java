 ...
 *     <li>firstBlock : premier bloc jouant le rôle d'en-têtes, avant les données chiffrées, contenant la secretKey et l'initializationVector chiffrés avec la clé publique</li>
 * </ul>
 *
 * <p>
 *     Liste des différents noms d'algorithmes supportés par Java : <a href="https://docs.oracle.com/javase/9/docs/specs/security/standard-names.html">https://docs.oracle.com/javase/9/docs/specs/security/standard-names.html</a>
 * </p>
 */
@Builder
public class RudiAlgorithmSpec implements AlgorithmParameterSpec {
	 ...
	 */
	private static final Supplier<RudiAlgorithmSpecBuilder> TLS_AES_256_GCM_SHA384_TEMPLATE = () -> RudiAlgorithmSpec.builder()
	...
	public static final RudiAlgorithmSpec DEFAULT = TLS_AES_256_GCM_SHA384_TEMPLATE.get()
			.firstBlockSpec(FirstBlockSpec.builder()
					// On utilise exactement le même paramétrage que la librairie JavaScript "SubtleCrypto" : https://developer.mozilla.org/en-US/docs/Web/API/SubtleCrypto/encrypt#rsa-oaep
					...
					.cipherAlgorithmParams(new OAEPParameterSpec(
							"SHA-256",
							"MGF1",
	...
	public static final class FirstBlockSpec {
		...
		/**
		 * Algorithme ayant servi à générer la paire de clés (la clé publique et la clé privée)
		 * utilisée pour (dé)chiffrer le premier bloc
		 */
		...
		/**
		 * Algorithme utilisé pour chiffrer le premier bloc de données.
		 * La taille de ce bloc est égale à {@link #getFirstBlockSizeInBytes()}.
		 */
		final String cipherAlgorithm;

		/**
		 * Paramètres utilisés par {@link #cipherAlgorithm l'algorithme de chiffrement du premier bloc}.
		 * {@code null} si aucun paramètre nécessaire.
		 */
		@Nullable
		final AlgorithmParameterSpec cipherAlgorithmParams;

		/**
		 * Dans le cas d'une clé RSA, on peut calculer la taille des blocs chiffrés qu'elle produit.
		 * Cette taille est fixe quelque soit les données chiffrées.
		 */
	...
	/**
	 * Clé secrète utilisé pour chiffrer tous les autres blocs après le premier.
	 * Elle, et le vecteur d'initialisation, sont chiffrés dans le premier bloc
