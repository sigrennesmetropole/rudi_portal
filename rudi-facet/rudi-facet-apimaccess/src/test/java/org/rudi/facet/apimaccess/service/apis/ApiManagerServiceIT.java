...
class ApiManagerServiceIT {
	...
	void testScenario() throws APIManagerException {
		...
		LimitingPolicy subscriptionLimitingPolicy = policyService.getSubscriptionLimitingPolicy("Gold");
		Assertions.assertNotNull(subscriptionLimitingPolicy);

		// création d'un api
		APIDescription apiDescription = new APIDescription().globalId(UUID.randomUUID()).responseMediaType("text/csv")
				.endpointUrl("https://demo.dataverse.org/api/access/datafile/1821528")
				.name("api-test-" + System.currentTimeMillis()).version("1.0.0")
				.interfaceContract(InterfaceContract.DOWNLOAD.getUrlPath()).providerUuid(UUID.randomUUID())
				.providerCode(RandomStringUtils.randomAlphabetic(10)).mediaUuid(UUID.randomUUID());
		api = apIsService.createOrUnarchiveAPI(apiDescription);
		Assertions.assertNotNull(api);

		// publication de l'api
		APIWorkflowResponse apiWorkflowResponse = apIsService.updateAPILifecycleStatus(api.getId(),
				APILifecycleStatusAction.PUBLISH);
		Assertions.assertEquals(APIWorkflowResponse.WorkflowStatusEnum.APPROVED,
				apiWorkflowResponse.getWorkflowStatus());
		...
		// recherche de l'api
		APISearchCriteria apiSearchCriteria = new APISearchCriteria().limit(10).offset(0)
		...
		List<LimitingPolicy> throttlePolicies = apIsService.getAPISubscriptionPolicies(apiGet.getId());
		...
		LimitingPolicy applicationPolicy = policyService.getApplicationLimitingPolicy(
				applicationLimitingPolicies.getList().get(0).getName(), adminRegistrationId);
		Assertions.assertNotNull(subscriptionLimitingPolicy);

		// Création d'une application
		application = applicationService
				.createApplication(
						new Application().name(RandomStringUtils.randomAlphabetic(10))
								.description("application de test").throttlingPolicy(applicationPolicy.getName()),
						adminRegistrationId);

		// récupération d'une application
		Application applicationGet = applicationService.getApplication(application.getApplicationId(),
				adminRegistrationId);
		Assertions.assertNotNull(applicationGet);

		// recherche de l'application
		Applications applications = applicationService.searchApplication(
				new ApplicationSearchCriteria().limit(10).offset(0).name(application.getName()), adminRegistrationId);
		Assertions.assertTrue(applications.getCount() > 0);

		// création d'une souscription
		Subscription applicationAPISubscription = applicationService.subscribeAPI(new Subscription().apiId(api.getId())
				.applicationId(application.getApplicationId()).throttlingPolicy(subscriptionLimitingPolicy.getName()),
				adminRegistrationId);

		// Recherche des souscriptions pour l'application
		SubscriptionList applicationAPISubscriptions = applicationService
				.searchApplicationAPISubscriptions(new DevPortalSubscriptionSearchCriteria().limit(10).offset(0)
						.applicationId(application.getApplicationId()), adminRegistrationId);
		...
		Assertions.assertEquals(HasSubscriptionStatus.SUBSCRIBED,
				applicationService.hasSubscribeAPI(api.getId(), application.getApplicationId(), adminRegistrationId));
		...
		SubscriptionList applicationAPISubscriptionsAfterRemove = applicationService
				.searchApplicationAPISubscriptions(new DevPortalSubscriptionSearchCriteria().limit(10).offset(0)
						.applicationId(application.getApplicationId()), adminRegistrationId);
	...
	void testClean() throws APIManagerException {
		if (!StringUtils.isEmpty(application.getApplicationId())) {
			...
			try {
				...
				assertThat(e).isInstanceOf(APIManagerException.class).hasCauseInstanceOf(APIManagerHttpException.class)
						.hasRootCauseInstanceOf(APIManagerHttpException.class);
				final var cause = e.getCause();
				assertThat(cause).hasMessageContaining("404").hasMessageContainingAll("Requested application with Id",
						"not found");
			}
		}
		if (!StringUtils.isEmpty(api.getId())) {
			...
			try {
				...
				assertThat(e).isInstanceOf(APIManagerException.class).hasCauseInstanceOf(APIManagerHttpException.class)
						.hasRootCauseInstanceOf(APIManagerHttpException.class);
				final var cause = e.getCause();
				assertThat(cause).hasMessageContaining("404").hasMessageContainingAll("Requested API with Id",
						"not found");
			}
		}
	}
