...
@Validated
@Tag(name = "undeploy-api", description = "the undeploy-api API")
public interface UndeployApiApi {
	...
	/**
	 * POST /undeploy-api : Undeploy the API in the gateway This operation is used to undeploy an API in the gateway. If the Tenant domain is not provided
	 ...
	 * @return OK. API successfully undeployed from the Gateway. (status code 200) or Bad Request. Invalid request or validation error (status code 400)
	 *         or Not Found. Requested API does not exist. (status code 404)
	 */
	@Operation(operationId = "undeployAPI", summary = "Undeploy the API in the gateway", responses = {
			@ApiResponse(responseCode = "200", description = "OK. API successfully undeployed from the Gateway. ", content = @Content(mediaType = "application/json", schema = @Schema(implementation = DeployResponse.class))),
	...
	@RequestMapping(method = RequestMethod.POST, value = "/api/am/gateway/v2/undeploy-api", produces = {
			"application/json" })
	default ResponseEntity<DeployResponse> undeployAPI(
