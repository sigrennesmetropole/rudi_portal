...
abstract class ClientRegisterer<S extends ClientRegistrationResponse> {
	...
	/**
	 * Applique le patch pour faire fonctionner le register de l'utilisateur "anonymous" dans le cas où WSO2 le traite injustement en tant que
	 * "PRIMARY/anonymous" au lieu de "RUDI/anonymous". Cf. erreur "RegistrationServiceImpl User anonymous does not have any of subscribe/create/publish
	 * privileges to create an OAuth app" dans les commentaires de la RUDI-3042.
	 */
	...
	@Nonnull
	public ClientRegistration getRegistration(String username, String password)
	...
	public ClientRegistration register(String username, String password)
		...
		try {
			...
			registrationResponse = clientRegistrationperationAPI.register(usernameForApiManager, password,
					registrationRequest);
			...
			if (cause instanceof ApplicationAlreadyExistException) {
				...
				log.info("Application already exists for clientName "
						+ applicationAlreadyExistException.getClientName());
	...
	public RegistrationRequest buildRegistrationRequest(String username) {
		final var clientName = StringUtils.contains(username, "/") ? StringUtils.substringAfterLast(username, "/")
				: username;
		return RegistrationRequestV11.builder().redirectUris(List.of("www.google.lk")).clientName(clientName)
