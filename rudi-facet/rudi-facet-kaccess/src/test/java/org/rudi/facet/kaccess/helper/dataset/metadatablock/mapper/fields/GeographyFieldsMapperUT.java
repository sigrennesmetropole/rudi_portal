...
class GeographyFieldsMapperUT {
	...
	private final GeographyPrimitiveFieldsMapper geographyPrimitiveFieldsMapper = new GeographyPrimitiveFieldsMapper(
			fieldGenerator, jsonResourceReader.getObjectMapper(), null);
	private final GeographyFieldsMapper geographyFieldsMapper = new GeographyFieldsMapper(fieldGenerator,
			geographyPrimitiveFieldsMapper);

	@ParameterizedTest
	@ValueSource(strings = { "geography", "geography-minimum-metadataToFields", "geography-empty-properties",
	...
	void metadataToFields(final String filesBaseName) throws IOException, JSONException, DataverseMappingException {
		...
		assertThat(fields).isJsonEqualToContentOf(getFieldsPath(filesBaseName));
	}
	...
	@ParameterizedTest
	@ValueSource(strings = { "geography", "geography-minimum-fieldsToMetadata", "geography-non-empty-properties" })
	void fieldsToMetadata(final String filesBaseName) throws IOException, JSONException, DataverseMappingException {
		final List<DatasetMetadataBlockElementField> fields = jsonResourceReader.readList(getFieldsPath(filesBaseName),
				DatasetMetadataBlockElementField.class);

		final Metadata metadata = new Metadata();
		geographyFieldsMapper.fieldsToMetadata(new RootFields(fields), metadata);

		assertThat(metadata).isJsonEqualToContentOf(getMetadataPath(filesBaseName));
	}

	@Test
	void metadataToFields_unknownGeoJsonType() {
		assertThatThrownBy(
				() -> jsonResourceReader.read(getMetadataPath("geography-unknown-geojsontype"), Metadata.class))
	...
	void metadataToFields_GeoJsonAsString() {
		assertThatThrownBy(() -> jsonResourceReader.read(getMetadataPath("geography-geojsonasstring"), Metadata.class))
				.isInstanceOf(InvalidTypeIdException.class).hasMessageContaining("missing type id");
	}
}
