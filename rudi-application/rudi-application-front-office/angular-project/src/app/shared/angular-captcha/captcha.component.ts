...
export class CaptchaComponent implements OnInit {
  ...
  validateUnsafe(callback: (isHuman: boolean) => void): void {
    const userInput = this.captchaService.botdetectInstance.userInput;
    const captchaCode = userInput.value;
  ...
  loadScriptIncludes(): void {
    const captchaId = this.elementRef.nativeElement.querySelector('#BDC_VCID_' + this.captchaStyleName).value;
