 ...
 *
 */
public class RudiUserStoreManager extends AbstractUserStoreManager {
	...
	private static final Log LOGGER = LogFactory.getLog(RudiUserStoreManager.class);
	...
	public String[] doListUsers(String filter, int maxItemLimit) throws UserStoreException {
		...
		final int searchTime = getSearchTime(UserCoreConstants.MAX_SEARCH_TIME, UserCoreConstants.RealmConfig.PROPERTY_MAX_SEARCH_TIME);
		...
		try {
			...
			filter = trim(filter, filter.trim().length() != 0, filter.trim(), "*", "%");
	...
	private int getSearchTime(int maxSearchTime, String propertyMaxSearchTime) {
		...
		try {
			searchTime = Integer
					.parseInt(realmConfig.getUserStoreProperty(propertyMaxSearchTime));
		} catch (Exception e) {
	...
	private int getGivenMax() {
		return getSearchTime(UserCoreConstants.MAX_USER_ROLE_LIST, UserCoreConstants.RealmConfig.PROPERTY_MAX_USER_LIST);
	}

	private String getUserFilterQuery(String caseSensitiveQueryPropertyName, String caseInsensitiveQueryPropertyName) {

		return getSqlStatement(isCaseSensitiveUsername(), caseSensitiveQueryPropertyName, caseInsensitiveQueryPropertyName);
	}
	...
	protected String[] doGetSharedRoleNames(String tenantDomain, String filter, int maxItemLimit)
		...
		try {
			...
			try {
				...
				String errorMessage = "Error while fetching roles from JDBC user store for tenant domain : "
	...
	public String[] getProfileNames(String userName) throws UserStoreException {
		...
		final String sqlStmt = getSqlStatement(isCaseSensitiveUsername(), JDBCRealmConstants.GET_PROFILE_NAMES_FOR_USER, JDBCCaseInsensitiveConstants.GET_PROFILE_NAMES_FOR_USER_CASE_INSENSITIVE);
		if (sqlStmt == null) {
	...
	public int getUserId(String username) throws UserStoreException {
		final String sqlStmt = getSqlStatement(isCaseSensitiveUsername(), JDBCRealmConstants.GET_USERID_FROM_USERNAME, JDBCCaseInsensitiveConstants.GET_USERID_FROM_USERNAME_CASE_INSENSITIVE);
		if (sqlStmt == null) {
	...
	public int getTenantId(String username) throws UserStoreException {
		...
		final String sqlStmt = getSqlStatement(isCaseSensitiveUsername(), JDBCRealmConstants.GET_TENANT_ID_FROM_USERNAME, JDBCCaseInsensitiveConstants.GET_TENANT_ID_FROM_USERNAME_CASE_INSENSITIVE);
		if (sqlStmt == null) {
			throw new UserStoreException("The sql statement for retrieving ID is null");
		}
		int id = -1;
	...
	public Map<String, String> getUserPropertyValues(String userName, String[] propertyNames, String profileName)
		...
		try {
			dbConnection = getDBConnection();
			final String sqlStmt = getSqlStatement(isCaseSensitiveUsername(), JDBCRealmConstants.GET_PROPS_FOR_PROFILE, JDBCCaseInsensitiveConstants.GET_PROPS_FOR_PROFILE_CASE_INSENSITIVE);
	...
	public boolean doCheckExistingUser(String userName) throws UserStoreException {
		...
		final String sqlStmt = getSqlStatement(isCaseSensitiveUsername(), JDBCRealmConstants.GET_IS_USER_EXISTING, JDBCCaseInsensitiveConstants.GET_IS_USER_EXISTING_CASE_INSENSITIVE);
		if (sqlStmt == null) {
		...
		if (Boolean.parseBoolean(isUnique) && !CarbonConstants.REGISTRY_ANONNYMOUS_USERNAME.equals(userName)) {
			final String uniquenesSql = getSqlStatement(isCaseSensitiveUsername(), JDBCRealmConstants.USER_NAME_UNIQUE, JDBCCaseInsensitiveConstants.USER_NAME_UNIQUE_CASE_INSENSITIVE);
	...
	private PreparedStatement doAuthenticateCreateStatement(Connection dbConnection, String userName)
		...
		final String sqlstmt = getSqlStatement(isCaseSensitiveUsername(), JDBCRealmConstants.SELECT_USER, JDBCCaseInsensitiveConstants.SELECT_USER_CASE_INSENSITIVE);
	...
	protected String getProperty(Connection dbConnection, String userName, String propertyName, String profileName)
		...
		final String sqlStmt = getSqlStatement(isCaseSensitiveUsername(), JDBCRealmConstants.GET_PROP_FOR_PROFILE, JDBCCaseInsensitiveConstants.GET_PROP_FOR_PROFILE_CASE_INSENSITIVE);
		if (sqlStmt == null) {
	...
	public void addRememberMe(String userName, String token) throws org.wso2.carbon.user.api.UserStoreException {
		...
		try {
			...
			if (values != null && values.length > 0 && values[0].length() > 0) {
				// udpate
				DatabaseUtil.updateDatabase(dbConnection, HybridJDBCConstants.UPDATE_REMEMBERME_VALUE_SQL, token,
						createdTime, userName, tenantId);
			} else {
				// add
				DatabaseUtil.updateDatabase(dbConnection, HybridJDBCConstants.ADD_REMEMBERME_VALUE_SQL, userName, token,
						createdTime, tenantId);
			}
			dbConnection.commit();
		} catch (SQLException e) {
		...
		if (value != null && createdTime != null) {
			...
			if (expDate.before(nowDate)) {
				...
				if (value.equals(token)) {
					...
					LOGGER.debug("Remember me token in DB and token in request are different !!");
					isValid = false;
				}
			}
		}
		...
		try {
			...
			if (isCaseSensitiveUsername()) {
				...
				for (int i = 0; i < users.size(); i++) {
					...
					users.set(i, users.get(i).replace("'", "''"));
	...
	protected String[] doGetSharedRoleListOfUser(String userName, String tenantDomain, String filter)
		...
		if (isSharedGroupEnabled()) {
			// shared roles
			final String sqlStmt = getSqlStatement(isCaseSensitiveUsername(), JDBCRealmConstants.GET_SHARED_ROLES_FOR_USER, JDBCCaseInsensitiveConstants.GET_SHARED_ROLES_FOR_USER_CASE_INSENSITIVE);
	...
	public long doCountRoles(String filter) throws UserStoreException {
		...
		if (filter.startsWith(UserCoreConstants.INTERNAL_DOMAIN)) {
			...
			sqlStmt = getSqlStatement(filter.startsWith(UserCoreConstants.APPLICATION_DOMAIN), JDBCRealmConstants.COUNT_APPLICATION_ROLES, JDBCRealmConstants.COUNT_ROLES);
		}
	...
	protected int doGetListUsersCount(String filter) throws UserStoreException {
		...
		try {
			...
			filter = trim(filter, StringUtils.isNotEmpty(filter.trim()), filter.trim().replace("*", "%"), "?", "_");
			...
			sqlStmt = getSqlStatement(isCaseSensitiveUsername(), JDBCRealmConstants.GET_USER_FILTER_PAGINATED_COUNT, JDBCCaseInsensitiveConstants.GET_USER_FILTER_CASE_INSENSITIVE_PAGINATED_COUNT);
	...
	private String getSqlStatement(boolean caseSensitiveUsername, String getUserFilterPaginatedCount, String getUserFilterCaseInsensitivePaginatedCount) {
		String sqlStmt;
		if (caseSensitiveUsername) {
			sqlStmt = realmConfig.getUserStoreProperty(getUserFilterPaginatedCount);
		} else {
			sqlStmt = realmConfig.getUserStoreProperty(
					getUserFilterCaseInsensitivePaginatedCount);
		}
		return sqlStmt;
	}
	...
	// TODO découper cette méthode trop complexe
	// TODO éviter de dupliquer le SQL dans cette méthode
	...
	private void buildGroupWhereConditions(SqlBuilder sqlBuilder, String operation, String value) {
		...
		if (ExpressionOperation.EQ.toString().equals(operation)) {
			...
			sqlBuilder.where(R_UM_ROLE_NAME_LIKE_QUERY, "%" + value);
	...
	private void buildGroupConditionWithOROperator(SqlBuilder sqlBuilder, String operation, String value) {
		...
		if (ExpressionOperation.EQ.toString().equals(operation)) {
			...
			sqlBuilder.updateSqlWithOROperation(R_UM_ROLE_NAME_LIKE_QUERY, "%" + value);
		} else if (ExpressionOperation.CO.toString().equals(operation)) {
			sqlBuilder.updateSqlWithOROperation(R_UM_ROLE_NAME_LIKE_QUERY, "%" + value + "%");
		} else if (ExpressionOperation.SW.toString().equals(operation)) {
			sqlBuilder.updateSqlWithOROperation(R_UM_ROLE_NAME_LIKE_QUERY, value + "%");
	...
	private void buildClaimWhereConditions(SqlBuilder sqlBuilder, String attributeName, String operation,
		...
		if (ExpressionOperation.EQ.toString().equals(operation)) {
			...
			sqlBuilder.where(UA_UM_ATTR_VALUE_LIKE_QUERY, "%" + attributeValue);
		} else if (ExpressionOperation.CO.toString().equals(operation)) {
			sqlBuilder.where(UA_UM_ATTR_VALUE_LIKE_QUERY, "%" + attributeValue + "%");
		} else if (ExpressionOperation.SW.toString().equals(operation)) {
			sqlBuilder.where(UA_UM_ATTR_VALUE_LIKE_QUERY, attributeValue + "%");
	...
	private void buildClaimConditionWithOROperator(SqlBuilder sqlBuilder, String attributeName, String operation,
		...
		if (ExpressionOperation.EQ.toString().equals(operation)) {
			...
			sqlBuilder.updateSqlWithOROperation(UA_UM_ATTR_VALUE_LIKE_QUERY, "%" + attributeValue);
		} else if (ExpressionOperation.CO.toString().equals(operation)) {
			sqlBuilder.updateSqlWithOROperation(UA_UM_ATTR_VALUE_LIKE_QUERY, "%" + attributeValue + "%");
		} else if (ExpressionOperation.SW.toString().equals(operation)) {
			sqlBuilder.updateSqlWithOROperation(UA_UM_ATTR_VALUE_LIKE_QUERY, attributeValue + "%");
	...
	protected HttpClient getHttpClient() throws GeneralSecurityException, IOException {
		if (httpClient == null) {
