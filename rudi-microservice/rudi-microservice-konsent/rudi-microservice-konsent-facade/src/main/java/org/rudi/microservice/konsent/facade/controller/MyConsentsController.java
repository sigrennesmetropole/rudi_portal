...
public class MyConsentsController implements MyConsentsApi {
	...
	public ResponseEntity<PagedConsentList> searchMyConsents(OffsetDateTime acceptDateMin, OffsetDateTime acceptDateMax,
			OffsetDateTime expirationDateMin, OffsetDateTime expirationDateMax, List<UUID> owners,
			List<UUID> treatments, Integer offset, Integer limit, String order) throws Exception {
		ConsentSearchCriteria searchCriteria = new ConsentSearchCriteria().treatmentOwnerUuids(owners)
				.treatmentUuids(treatments).acceptDateMin(acceptDateMin).acceptDateMax(acceptDateMax)
				.expirationDateMin(expirationDateMin).expirationDateMax(expirationDateMax);
