...
public class SelfdataServiceTest {
	...
	@Test
	void test_get_gdata_data_500_on_searchApplicationWso2Error() throws DataverseAPIException, ApplicationOperationException {
		UUID randomUuid = UUID.randomUUID();
		when(datasetService.getDataset(randomUuid)).thenReturn(new Metadata());
		...
		when(registrationHelper.getOrCreateApplicationForUser(correctUser.getLogin())).thenThrow(ApplicationOperationException.class);
		assertThrows(AppServiceException.class, () -> selfdataService.getGdataData(randomUuid));
	}

	@Test
	void test_get_gdata_data_ok_on_ok_data() throws DataverseAPIException, AppServiceException {
