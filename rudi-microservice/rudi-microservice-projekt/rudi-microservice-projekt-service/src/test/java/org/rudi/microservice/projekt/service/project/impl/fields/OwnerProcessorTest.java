...
class OwnerProcessorTest {
	...
	void process_user_create_himself() {
		...
		assertThatCode(() -> {
			ownerProcessor.process(projectToCreate, null);
		})
	...
	void process_user_update_himself() {
		...
		assertThatCode(() -> {
			ownerProcessor.process(modifiedProject, existingProject);
		})
				.as("Le porteur de projet peut créer un projet en son nom")
				.doesNotThrowAnyException();
	}
	...
	@Test
	void process_user_update_someoneElsesProject() {
		...
		assertThatThrownBy(() -> {
			ownerProcessor.process(modifiedProject, existingProject);
				...
				.as("Le porteur de projet ne peut pas modifier le projet de quelqu'un d'autre")
				.isInstanceOf(AppServiceForbiddenException.class)
	...
	void process_organization_create_notMember() throws GetOrganizationMembersException {
		...
		assertThatThrownBy(() -> {
			ownerProcessor.process(projectToCreate, null);
				...
				.as("Le porteur de projet peut créer un projet au nom d'une organisation à laquelle il appartient")
				.isInstanceOf(AppServiceForbiddenException.class)
				.hasMessage("Authenticated user cannot create project in the name of an organization they do not belong to");
	}
	...
	@Test
	void process_organization_update_notMember() throws GetOrganizationMembersException {
		...
		assertThatThrownBy(() -> {
			ownerProcessor.process(modifiedProject, existingProject);
				...
				.isInstanceOf(AppServiceForbiddenException.class)
				.hasMessage("Authenticated user cannot update project in the name of an organization they do not belong to");
	}
