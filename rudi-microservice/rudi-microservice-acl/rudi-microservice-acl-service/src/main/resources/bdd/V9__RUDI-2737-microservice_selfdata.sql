...
INSERT INTO "user" (uuid, company, firstname, lastname, login, password, type)
...
VALUES ((SELECT id FROM acl_data.user WHERE login = 'selfdata'), (SELECT id FROM acl_data.role WHERE code = 'MODULE')),
       ((SELECT id FROM acl_data.user WHERE login = 'selfdata'),
