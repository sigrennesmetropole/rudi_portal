...
insert into acl_data.user_role( user_fk, role_fk) values
	( (select id from acl_data.user_ where login = 'kos'), (select id from acl_data.role where code = 'MODULE')),
	( (select id from acl_data.user_ where login = 'kos'), (select id from acl_data.role where code = 'MODULE_KOS_ADMINISTRATOR'));